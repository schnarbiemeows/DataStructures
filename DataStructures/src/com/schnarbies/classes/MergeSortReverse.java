...
public class MergeSortReverse {
	...
	public static void main(String[] args) {
		...
		/* 
		 * 2 phase sort: split and merge
		 * split -
		 * start with an unsorted array.
		 * divide the array into 2 arrays, which are unsorted.
		 ...
		 * keep splitting until all the arrays have only 1 elements, these are sorted by default
		 *  
		 *  merge - 
		 *  merge every left/right pair of sibling arrays into a sorted array
		 *  after the first merge, we have a bunch of 2 element sorted arrays
		 ...
		 *  we create a temporary array large enough to hold all of the elements in the arrays
		 *  we are merging
		 *  we set i to the first index of the left array and j to the first index of the 
		 *  right array
		 *  we compare left[i] and right[j]. if left is smaller, we copy it to the temp
		 *  array and increment i by 1. if right is smaller, we copy it to the temp array and
		 *  increment j by 1
		 *  we repeat this process until all elements in the two arrays have been processed.
		 *  at this point, the temp array contains the merged values in sorted order.
		 *  we then copy this temp array back to the original input array, at the correct
		 *  positions.
		...
		public static void merge(int[] input, int start, int mid, int end) {
			...
			if(input[mid -1] >= input[mid]) {
				// this is just an optimization; see if we actually have to do anything
				// so if the rightmost element of the left array is > the leftmost element
				// of the right array, there is no need to do anything
			...
			//System.arraycopy(input, i, input, start+tempIndex, mid-i); // i don't understand this
