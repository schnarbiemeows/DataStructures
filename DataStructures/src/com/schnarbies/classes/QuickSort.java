	...
	public static void main(String[] args) {
		...
		public static int partition(int[] input, int start, int end) {
			...
			while(i < j) {
				...
				while(i < j && input[--j] >= pivot) System.out.println("decrementing j to => " + j);
				if(i < j) {
					System.out.println("copying value : " + input[j] + " from the right at index: " + j + " to the left at index: "+i+" , overwriting "+input[i]);
					input[i] = input[j];
				...
				// NOTE: empty loop body
				while(i < j && input[++i] <=pivot) System.out.println("incrementing i to => " + i);
				if(i < j) {
					System.out.println("copying value : " + input[i] + " from the left at index: " + i + " to the right at index: "+j+" , overwriting "+input[j]);
					input[j] = input[i];
					System.out.print("Our new array is : ");
					for(int x = start; x < end; x++) {
						System.out.print(input[x]+" ");
			...
			System.out.println("j("+j+") is no longer less than i("+i+"), so we are done with this partitioning method, setting index: "+j+" to "+pivot+" overwriting "+input[j]);
			input[j] = pivot;
